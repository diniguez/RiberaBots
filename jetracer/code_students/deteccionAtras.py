...
def lidar_callback(msg):
    ...
    inicio = int(n * (150/360))
    fin = int(n * (210/360))
    sector = msg.ranges[inicio:fin]
    ...
    if validos:
        ...
        if obstaculo_detectado:
            print(f"OBSTÁCULO DETRÁS a {minimo:.2f} m!")
...
try:
    while not rospy.is_shutdown():
        if obstaculo_detectado:
            ...
            print(f"EXPLORANDO MARCHA ATRÁS -> vel={x:.2f} giro={yaw:.2f} tiempo={dur:.1f} s")
            velocidad(x, yaw)
            time.sleep(dur)
            velocidad(0.0)
            time.sleep(0.3)
